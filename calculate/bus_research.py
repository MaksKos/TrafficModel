...
file_name = 'bus'
# variable 
steps = [100, 100, 100] #[1000, 1000, 200, 100, 50, 50, 50, 100, 100]
n_station = [15, 10, 5] #[0, 1, 5, 10, 20, 5, 10, 5, 20]
#############################################################
...
def calc_data(step_stations: int, amount_station:int, cluster: Client):
    ...
    buses_arr = np.arange(1, MAX_BUSES, STEP)
    futs = []
...
def main():
    ...
    for step_station, amount_stations in zip(steps, n_station):
        ...
        name = directory + file_name + f"bus_{MAX_BUSES-1}_step_{step_station}_station_{amount_stations}.csv"
        tabel.to_csv(name)
